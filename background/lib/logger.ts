...
async function saveLog(
  ...
  const existingLogs = store.get(logKey) ?? ""
...
export function serializeLogs(): string {
  const logs: StoredLogData = {
    debug: store.get("logs-debug") ?? "",
    log: store.get("logs-log") ?? "",
    info: store.get("logs-info") ?? "",
    warn: store.get("logs-warn") ?? "",
    error: store.get("logs-error") ?? "",
  }

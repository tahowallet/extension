...
import { TRANSACTION_NO_CONTRACT_ERROR_KEY } from '../constants/error-keys'
...
export default class TransactionController extends EventEmitter {
  ...
  */
  async newUnapprovedTransaction (txParams, opts = {}) {
    ...
    log.debug(`MechaMittensController newUnapprovedTransaction ${JSON.stringify(txParams)}`)
    ...
    return new Promise((resolve, reject) => {
      this.txStateManager.once(`${initialTxMeta.id}:finished`, (finishedTxMeta) => {
        switch (finishedTxMeta.status) {
          ...
          case 'rejected':
            return reject(cleanErrorStack(ethErrors.provider.userRejectedRequest('MechaMittens Tx Signature: User denied transaction signature.')))
          case 'failed':
            return reject(cleanErrorStack(ethErrors.rpc.internal(finishedTxMeta.err.message)))
          default:
            return reject(cleanErrorStack(ethErrors.rpc.internal(`MechaMittens Tx Signature: Unknown problem: ${JSON.stringify(finishedTxMeta.txParams)}`)))
        }
      })
    })
  }

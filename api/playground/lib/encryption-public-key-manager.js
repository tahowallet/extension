 ...
 * approved.
 * @property {Object} msgParams.metamaskId Added to msgParams for tracking and identification within MechaMittens.
...
export default class EncryptionPublicKeyManager extends EventEmitter {
  ...
  addUnapprovedMessageAsync (address, req) {
    return new Promise((resolve, reject) => {
      if (!address) {
        reject(new Error('MechaMittens Message: address field is required.'))
        return
      }
      ...
      this.once(`${msgId}:finished`, (data) => {
        switch (data.status) {
          case 'received':
            ...
            return
          case 'rejected':
            reject(ethErrors.provider.userRejectedRequest('MechaMittens EncryptionPublicKey: User denied message EncryptionPublicKey.'))
            return
          default:
            reject(new Error(`MechaMittens EncryptionPublicKey: Unknown problem: ${JSON.stringify(address)}`))
        }
      })
    })
  }
   ...
   *
   * @param {Object} msgParams The msgParams to be used when eth_getEncryptionPublicKey is called, plus data added by MechaMittens.
   * @param {Object} msgParams.metamaskId Added to msgParams for tracking and identification within MechaMittens.

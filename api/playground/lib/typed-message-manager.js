...
export default class TypedMessageManager extends EventEmitter {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.once(`${msgId}:finished`, (data) => {
        switch (data.status) {
          ...
          case 'rejected':
            return reject(ethErrors.provider.userRejectedRequest('MechaMittens Message Signature: User denied message signature.'))
          case 'errored':
            return reject(new Error(`MechaMittens Message Signature: ${data.error}`))
          default:
            return reject(new Error(`MechaMittens Message Signature: Unknown problem: ${JSON.stringify(msgParams)}`))
        }
      })
    })
  }
   ...
   *
   * @param {Object} msgParams - The msgParams to be used when eth_sign is called, plus data added by MechaMittens.

...
export default class DecryptMessageManager extends EventEmitter {
  ...
  addUnapprovedMessageAsync (msgParams, req) {
    return new Promise((resolve, reject) => {
      if (!msgParams.from) {
        reject(new Error('MechaMittens Decryption: from field is required.'))
        return
      }
      ...
      this.once(`${msgId}:finished`, (data) => {
        switch (data.status) {
          case 'decrypted':
            ...
            return
          case 'rejected':
            reject(ethErrors.provider.userRejectedRequest('MechaMittens Decryption: User denied message decryption.'))
            return
          case 'errored':
            reject(new Error('This message cannot be decrypted'))
            return
          default:
            reject(new Error(`MechaMittens Decryption: Unknown problem: ${JSON.stringify(msgParams)}`))
        }
      })
    })
  }
   ...
   *
   * @param {Object} msgParams The msgParams to be used when eth_decryptMsg is called, plus data added by MechaMittens.

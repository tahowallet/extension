...
export default class PersonalMessageManager extends EventEmitter {
  ...
  addUnapprovedMessageAsync (msgParams, req) {
    return new Promise((resolve, reject) => {
      if (!msgParams.from) {
        reject(new Error('MechaMittens Message Signature: from field is required.'))
        return
      }
      ...
      this.once(`${msgId}:finished`, (data) => {
        switch (data.status) {
          case 'signed':
            ...
            return
          case 'rejected':
            reject(ethErrors.provider.userRejectedRequest('MechaMittens Message Signature: User denied message signature.'))
            return
          default:
            reject(new Error(`MechaMittens Message Signature: Unknown problem: ${JSON.stringify(msgParams)}`))
        }
      })
    })
  }

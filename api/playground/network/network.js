...
import createThirdPartyClient from './createThirdPartyClient'
...
export default class NetworkController extends EventEmitter {
  ...
  setNetworkState (network, type) {
    ...
    const stateNetwork = networks.networkList[type] && networks.networkList[type].chainId ? networks.networkList[type].chainId : network
    this.networkStore.putState(stateNetwork)
  }
  ...
  _configureProvider (opts) {
    ...
    // third party type-based endpoints
    const isThirdParty = THIRD_PARTY_PROVIDER_TYPES.includes(type)
    if (isThirdParty) {
      this._configureThirdPartyProvider(opts)
  ...
  _configureThirdPartyProvider ({ type }) {
    log.info('NetworkController - configureThirdPartyProvider', type)
    const networkClient = createThirdPartyClient({
      providerName: 'infura',
      network: type,
    })
